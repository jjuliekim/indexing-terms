...
# extract text from document and tokenize
def parse_file(path):
...
term_df_file = "Stemmed/term_df.json"
term_df = {}
term_cf_file = "Stemmed/term_cf.json"
term_cf = {}


# write catalog file for each index file
def write_catalog(index_dict, index_file):
    index_file_root = index_file[15:-4]
    catalog_file_base = "CatalogsStemmed/catalog_{}.json"
...
doc_num = 1
term_num = 1
index_file_base = "IndexesStemmed/index_{}.bin"
...
id_to_doc_file = "Stemmed/id_to_doc.json"
doc_to_id_file = "Stemmed/doc_to_id.json"
id_to_term_file = "Stemmed/id_to_term.json"
term_to_id_file = "Stemmed/term_to_id.json"
    ...
    with open(index_file, 'rb') as bin_file:
        for term, offset, size in catalog:
            ...
            if term in final_index.keys():
                # print("term", term)
                updated_data = {}
                # print("already in final[term]:", final_index[term])
                # print("want to add index_data", index_data)
...
for num, filename in enumerate(os.listdir("CatalogsStemmed")):
    merge_files(("CatalogsStemmed/catalog_index_" + str(num) + ".json"), ("IndexesStemmed/index_" + str(num) + ".bin"))
...
init_offset = 0
# write dict to binary file
with open("final_index_stemmed.bin", 'wb') as bin_file:
