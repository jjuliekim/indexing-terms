...
with open('NonStemmed/term_cf.json', 'r') as json_file:
    term_cf = json.load(json_file)
with open('NonStemmed/term_df.json', 'r') as json_file:
    term_df = json.load(json_file)

# get doc id and term id json info
with open('NonStemmed/doc_to_id.json', 'r') as json_file:
    doc_ids = json.load(json_file)
with open('NonStemmed/id_to_doc.json', 'r') as json_file:
    id_docs = json.load(json_file)
with open('NonStemmed/term_to_id.json', 'r') as json_file:
    term_ids = json.load(json_file)
with open('NonStemmed/id_to_term.json', 'r') as json_file:
...
try:
    with open('NonStemmed/doc_length.json', 'r') as json_file:
    ...
    with open('NonStemmed/doc_length.json', 'w') as file:
...
def run_tf_idf():
    for query_number, query_text in queries.items():
        ...
        with open('Resources/query_result_tf_idf.txt', 'a') as file:
...
# # testing catalog and index retrieval
# catalog_data = next((entry for entry in catalog if entry[0] == 1), None)
# with open(index_file, "rb") as file:
#     file.seek(catalog_data[1])
#     bin_data = file.read(catalog_data[2])
#     data = json.loads(bin_data)
# for term_id, doc_maps in data.items():
#     print(len(doc_maps.get("5976", [])))
#     print(len(doc_maps.get("8058", [])))
#     print(len(doc_maps.get("81786", [])))
#     print(len(doc_maps.get("3", [])))
...
def run_okapi_bm25():
    ...
    for query_number, query_text in queries.items():
        ...
        # add to output file after each query
        output_lines.sort(reverse=True, key=lambda x: x[0])
        with open('Resources/query_result_okapi_bm.txt', 'a') as file:
            for rank, (score, doc_id) in enumerate(output_lines[:1000], start=1):
                ranked_line = f"{query_number} Q0 {doc_id} {rank} {score} Exp\n"
                file.write(ranked_line)
...
def run_laplace():
    for query_number, query_text in queries.items():
        ...
        with open('Resources/query_result_laplace.txt', "a") as file:
